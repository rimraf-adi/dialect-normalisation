...
def load_and_sample_dialect_dataset(
    ...
    and samples `samples_per_dialect` items for each specified dialect.
    """
    if target_dialects is None:
        target_dialects = ["D1", "D2", "D4"]

    print(f"[Sampler] Reading metadata file: {meta_path}...")
    with open(meta_path, "r", encoding="utf-8") as f:
        data = json.load(f)

    entries = list(data.values()) if isinstance(data, dict) else data
    print(f"[Sampler] Loaded {len(entries):,} total utterances from metadata.")
    ...
    print(f"\n[Sampler] Sampling {samples_per_dialect:,} items per dialect (Target Dialects: {', '.join(target_dialects)})...")

    for dial in target_dialects:
        available_items = list(dialect_buckets[dial].values())
        print(f"  --> Dialect {dial}: {len(available_items):,} total unique sentences available.")
        ...
        if len(available_items) >= samples_per_dialect:
            # Take top 60% by distortion score + random sample remaining to maintain diversity
            ...
            print(f"      Warning: Dialect {dial} has fewer than {samples_per_dialect} unique sentences. Taking all {len(available_items)}.")
            sampled_dataset[dial] = available_items

        print(f"  --> Dialect {dial} Selected: {len(sampled_dataset[dial]):,} samples (Avg Distortion Score: {sum(x['distortion_score'] for x in sampled_dataset[dial])/len(sampled_dataset[dial]):.2f})")

    total_sampled = sum(len(v) for v in sampled_dataset.values())
    print(f"\n[Sampler] Final Sampled Dataset Total: {total_sampled:,} samples across {len(target_dialects)} dialects.")
