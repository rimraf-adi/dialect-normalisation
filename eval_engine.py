...
def evaluate_dataset(
    ...
    if not output_detailed_yaml_filename:
        output_detailed_yaml_filename = f"indic_conformer_{slug}_detailed.yaml"
    if not output_summary_yaml_filename:
        output_summary_yaml_filename = f"indic_conformer_{slug}_summary.yaml"

    output_detailed_path = output_dir / output_detailed_yaml_filename
    output_summary_path = output_dir / output_summary_yaml_filename
    ...
    for dec in decoder_modes:
        dec_res = run_evaluation_for_decoder(dec, model, samples, device, model_lang_code=model_lang_code)
