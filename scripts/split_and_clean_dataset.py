...
if not csv_files:
    print("No input CSV files found in data/synthetic_parallel!")
    sys.exit(1)

# Enhanced Dialect and Language Residue Patterns for filtering standard_text
RESIDUE_PATTERNS = {
...
# Hindi language leakage in standard_text
...
def detect_flaws(orig: str, trans: str) -> str:
    ...
    # Check 4: Question mark mismatch & lost question context
    ...
    # Check 5: Word count anomaly / drastic distortion
    ...
    # Check 6: Entity / Term hallucination or garbled patterns
        ...
        for row in reader:
            ...
            if flaw_reason:
                ...
                if dial == "D1":
                    ...
                    # Fallback to d1/d2/d4 based on dialect field or keep in d1
                    d1_rows.append(row)
...
with open(d1_path, "w", encoding="utf-8-sig", newline="") as f:
...
with open(d2_path, "w", encoding="utf-8-sig", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=fieldnames)
    writer.writeheader()
    writer.writerows(d2_rows)

# Write d4.csv
d4_path = data_dir / "d4.csv"
with open(d4_path, "w", encoding="utf-8-sig", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=fieldnames)
    writer.writeheader()
...
with open(flawed_path, "w", encoding="utf-8-sig", newline="") as f:
