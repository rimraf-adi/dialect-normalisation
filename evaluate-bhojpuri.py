...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate IndicConformer on RESPIN Bhojpuri Dataset.")
    parser.add_argument("--meta-file", type=str, default=None, help="Path to meta_test_bh.json metadata file")
    ...
    # Default paths for Bhojpuri
    if args.meta_file:
        meta_file = Path(args.meta_file)
        base_dir = Path(args.base_dir) if args.base_dir else meta_file.parent
    else:
        candidate1 = Path("IISc_RESPIN_test_bh/IISc_RESPIN_test_bh/meta_test_bh.json")
        candidate2 = Path("IISc_RESPIN_test_bh/meta_test_bh.json")
        if candidate1.exists():
            meta_file, base_dir = candidate1, Path("IISc_RESPIN_test_bh/IISc_RESPIN_test_bh")
        elif candidate2.exists():
            meta_file, base_dir = candidate2, Path("IISc_RESPIN_test_bh")
        else:
            meta_file, base_dir = candidate1, Path("IISc_RESPIN_test_bh/IISc_RESPIN_test_bh")
