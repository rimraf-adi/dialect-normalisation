...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate IndicConformer on RESPIN Chhattisgarhi Dataset.")
    parser.add_argument("--meta-file", type=str, default=None, help="Path to meta_test_ch.json metadata file")
    ...
    # Default paths for Chhattisgarhi
    if args.meta_file:
        meta_file = Path(args.meta_file)
        base_dir = Path(args.base_dir) if args.base_dir else meta_file.parent
    else:
        candidate1 = Path("IISc_RESPIN_test_ch/IISc_RESPIN_test_ch/meta_test_ch.json")
        candidate2 = Path("IISc_RESPIN_test_ch/meta_test_ch.json")
        if candidate1.exists():
            meta_file, base_dir = candidate1, Path("IISc_RESPIN_test_ch/IISc_RESPIN_test_ch")
        elif candidate2.exists():
            meta_file, base_dir = candidate2, Path("IISc_RESPIN_test_ch")
        else:
            meta_file, base_dir = candidate1, Path("IISc_RESPIN_test_ch/IISc_RESPIN_test_ch")
